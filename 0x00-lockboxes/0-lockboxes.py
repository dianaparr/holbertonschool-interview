...
def canUnlockAll(boxes):
    """ This exercise is the one example for the job interview.
        ...
        numbered sequentially from 0 to n - 1 and each box may contain keys to
        the other boxes.
    ...
    # method adds a counter to an iterable and returns it in a form
    # of enumerating object.
    for nextKey, box in enumerate(boxes):
        ...
        for key in box:
            ...
            if len(keysBoxes) == len(boxes) or len(
                      keysBoxes) == len(boxes) - 1:
                return True
